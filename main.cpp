...
public:
    ...
    void make_file(std::string name, unsigned long size=1, char* data=nullptr) {
    // mk
        ...
        if (data == nullptr) {
            for (int i=0; i<size; i++)
                strcpy(blocks[inodes[n].file_ptr+i].data, std::string(BLOCK_SIZE, '0').c_str());
        }
        else {
            for (int i=0; i<size; i++)
                for (int j=0; j<BLOCK_SIZE; j++)
                    blocks[inodes[n].file_ptr+i].data[j] = data[BLOCK_SIZE*i+j];
        }
    ...
    void load_file(std::string filename) {
        ...
        unsigned long i = 0;
        strcpy(data, std::string(MAX_FILE_SIZE, '\0').c_str());
        while(!file.eof() && i*BLOCK_SIZE < MAX_FILE_SIZE) {
            file >> one_block;
            for (int j=0; j<BLOCK_SIZE; j++)
                data[i * BLOCK_SIZE + j] = one_block[j];
            i++;
        }
        make_file(filename, i, data);
        file.close();
    }
    ...
    void save_file(std::string filename) {
        ...
        if (!file.good())
            throw "Could not open file";
        unsigned long f = get_inode_by_name(filename);
        for (int i=0; i<inodes[f].file_size; i++)
            file << blocks[inodes[f].file_ptr+i].data;
        file.close();
    }
...
//    fs.directory_info();
    FileSystem fs(1024);
    fs.load_file("test.txt");
    fs.file_info("test.txt");
    fs.save_file("test.txt");
//    fs.make_file("test", 5);
//    fs.info();
//    fs.file_info("test");
